...
export default async function ArticlePage(props: { params: Promise<{ locale: string, slug: string }> }) {
  ...
  return (
    <main className="container" style={{ padding: "6rem 0", maxWidth: "800px" }}>
      <Link href={`/${locale}/insights`} style={{ color: "var(--muted)", textDecoration: "none", marginBottom: "2rem", display: "inline-block" }}>
        ← Volver a Insights
      </Link>
      
      <span style={{ display: "block", color: "var(--accent)", fontSize: "0.9rem", fontWeight: 700, marginBottom: "1rem" }}>
        {new Date(article.createdAt).toLocaleDateString(locale === 'es' ? 'es-ES' : 'en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
      </span>
